...
# 머신러닝의 개념
# 컴퓨터 프로그램이 어떤 것에 대한 학습을 통해
# 기존의 모델이나 결과물을 개선하거나 예측하게끔
# 구축하는 과정
#
...
# 딥러닝 - 신경망을 층층이 쌓아서 문제를 해결하는 기법의 총칭
...
#데이터의 일부를 훈련 데이터, 나머지를 평가데이터로 분리
...
#데이터 분할 방법 1 : 연속적인 순서대로 나눔
# iris_train<-iris[1:105,]  #1행~105행까지 총 70%를 학습용으로 할당
# iris_test<-iris[106:150,]   #106~나머지 총 30%를 평가용으로 할당

#데이터 분할 방법 2: 무작위 추출대로 나눔
...
#학자는 붓꽃의 꽃잎petal과 꽃받침sepal의 폭과 길이를 알고 잇음

#한편, 전문식물학자는 붓꽃을 setosa, versicolor, virginica종으로
...
print(iris['target'][:5]) #iris 타겟(품종) 처음 5행만 출력
print(iris['target_names']) #iris 타겟(품종) 출력

#scikit-learn에서 train_test_spkit함수를 이용해서
#데이터 집합을 일정비율로 나눠 train/test로 작성
...
#dataframe 으로 변환 필요
#이렇게 분류가 되어 있어야만 knn을 사용할 수 있다.
from pandas.plotting import scatter_matrix
iris_df = pd.DataFrame(x_train)

#scatter_matrix(iris_df,c=y_train,figsize=(15,15),
#               marker='o', s=60,alpha=.8)

#비교해보기
scatter_matrix(iris_df,c=y_train,figsize=(15,15),marker='o')
plt.show()

#k-최근접 알고리즘에서 k는 가까운 이웃 '하나'가 아니라
#훈련데이터 중 새로운 데이터와
#가장 가까운 k개의 이웃을 찾는다는 의미
#머신러닝의 모든 모델은 scikit-learn의 Estimator 클래스에 구현
#KNN 알고리즘은 neighbors 모듈의 KNeighborsClassfier 클래스에 구현
...
print('test 데이터 정확도', knn.score(x_test,y_test))
...
#다형분류 : 3가지 이상 질문의 답 중 하나를 예측

#일반화/과대적합/과소적합
#일반화: 모델을 통해 처음 보는 데이터에 대해 정확히
#        예측할 수 있는 경우
# 예)요트구매 고객 정보 : 나이,보유차량수, 주택보유,
#           자녀수, 혼인여부, 애완견, 보트구매
#요트를 구매한 고객과 구매의사가 없는 고객 데이터를 토대로
#누가 요트를 구매할지 예측해보자

#과대적합(overfitting): "나이가 45세 이상, 자녀가 3이상,
#                     이혼하지 않은 고객은 요트를 구매할 것이다"
#                     너무 많은 특성을 이용해서 복잡한 모델을 만드는 경우
#과소적합(underfitting): "애완견이 있는 고객은 요트를 구매할 것이다."
